...
runformask <- function(outbase, tablefile, functionstring='', mask, maskval=1, columns=NA, libs=c(), functions=c(), colname=T, verbose=FALSE) {
	...
	voldims <- rev(minc.dimensions.sizes(mask))  # reverse order here (works when mincinfo yields dim order: z,y,xspace)
	vv <- array(v,dim=voldims) 
	firstmaskvoxels <- which(abs(vv-maskval)<0.5, arr.ind=T)
	firstmaskvoxel <- firstmaskvoxels[nrow(firstmaskvoxels),]-c(1,1,1)
	rm(v)
	rm(vv)

	datatable <<- read.csv(tablefile)
	x <<- mincGetVoxel(datatable$filename, rev(firstmaskvoxel))
