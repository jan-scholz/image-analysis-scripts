...
voxelBEST2 <- function(x, data=NULL) {
#voxelBEST <- function(x) {

if (!is.null(data)) d <- data
if (exists('datatable')) d <- datatable
...
getROPEcurve <- function(x, data=NULL, rope=c(0,0.15), nsdf=5) {
	...
	if (exists('settings')) {
		if ('rope' %in% names(settings)) rope <- settings[['rope']]
		#if ('nsdf' %in% names(settings)) nsdf <- settings[['nsdf']]
	}

	#stop(sprintf('%s',rope[2]))
	...
	# return ROPE radius where more than 95% overlap
	radius <- dfake$xx[p>0.95][1]
	if (is.na(radius)) radius <- rope[2]
	result <- radius
...
ROPEcurve_from_files <- function(filenames, mask, settings=list(), cpus=2, cores=2) {
	# settings contains parameters for getROPEcurve()
	...
	sfExport('getROPEcurve','settings')
